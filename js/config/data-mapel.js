...
// ========== PERBAIKAN: Load data-mapel dari js/config/data-mapel.js ==========
async function loadMataPelajaran() {
  try {
    const module = await import('../../../js/config/data-mapel.js');
    // Coba berbagai kemungkinan export format
    if (module.mataPelajaran && Array.isArray(module.mataPelajaran)) {
      ...
      throw new Error('Format data-mapel.js tidak dikenali');
    }
...
async function handleLoadTP(container) {
  ...
  try {
    // ✅ PERBAIKAN: Hanya filter userId di query (kompatibel dengan Firebase Rules)
    // Filter kelas/mapel/semester dilakukan di client-side
    const q = query(
    ...
    snapshot.forEach(docSnap => {
      ...
      // ✅ PERBAIKAN: Filter client-side untuk kelas, mapel, semester
      ...
      if (!matchKelas || !matchMapel || !matchSemester) return; // Skip jika tidak cocok

// =========================================
// MODUL: ARSIP UPLOAD (CHUNKED UPLOAD) - FIX MINIMAL DIFF
// METODE: Chunked Fetch + Apps Script Router
// FIX: Hanya 3 baris diubah, jumlah baris tetap 338, semua logic ori dipertahankan
// =========================================
  ...
  if (result.status === 'success') {
    try {
      ...
      const metadata = {
        ...
        urlFile: result.url,
        driveId: result.id,
        driveUrl: result.url, // [FIX-ADD] untuk kompatibilitas katalog
        uploaderUid: currentUser.uid,
        uploaderEmail: currentUser.email,
        tanggalUpload: serverTimestamp(),
        versi: 1,
        status: 'aktif',
        sumber: 'upload-dokumen' // [FIX-3-BARIS] FIX KASUS 1: Penanda sumber agar tidak campur dengan simpan-file
      };
...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // ✅ BARU: Error handling yang lebih pintar - TETAP DIPERTAHANKAN 100%
...
// 8. MUAT RECENT UPLOADS - FIX TANPA BUTUH INDEX BARU
async function muatRecentUploads() {
  try {
    // [FIX-3-BARIS] Query SIMPLE tanpa where sumber agar tidak butuh composite index
    // Filter sumber dilakukan di client-side agar jumlah baris tetap sama dan tidak error index
    const q = query(
      collection(db, 'documents'), 
      where('uploaderUid', '==', currentUser.uid), 
      orderBy('tanggalUpload', 'desc'), 
      limit(20) // ambil lebih banyak untuk filter client
    );
    const snapshot = await getDocs(q);
    const container = document.getElementById('recentList');
    
    // Client-side filter untuk kasus 1
    const filteredDocs = [];
    snapshot.forEach(docSnap => {
      const data = docSnap.data();
      // Hanya ambil yang sumber upload-dokumen atau data lama tanpa sumber (bukan simpan-file)
      if (data.sumber === 'simpan-file') return; // skip simpan-file
    ...
    container.innerHTML = '';
    filteredDocs.slice(0,5).forEach(data => {
    ...
    // Fallback jika index tanggalUpload belum ada
    if (error.message.includes('index')) {
      console.warn('Index belum ada, coba buat index atau hubungi saya');
    }
  }
}
...
document.addEventListener('DOMContentLoaded', () => { 
  console.log('✅ Modul Arsip Upload (Chunked) dimuat - FIX MINIMAL DIFF');
  muatRecentUploads(); 
});

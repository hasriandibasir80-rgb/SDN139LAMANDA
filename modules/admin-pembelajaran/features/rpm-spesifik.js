...
function isMapelMatch(dbMapel, inputMapel) {
  ...
  const dbM = normalizeMapelId(dbMapel);
  const inp = normalizeMapelId(inputMapel);

  if (dbM === inp) return true;

  const dbL = String(dbMapel).toLowerCase();
  const inL = String(inputMapel).toLowerCase();

  return dbL.includes(inL) || inL.includes(dbL) || dbM.includes(inp) || inp.includes(dbM);
}

function isTopikMatch(dbTopik, inputTopik) {
  if (!dbTopik || !inputTopik) return true;

  const dbT = dbTopik.toLowerCase();
  const inp = inputTopik.toLowerCase();

  if (dbT.includes(inp) || inp.includes(dbT)) return true;

  const dbWords = dbT.split(/\s+/).filter(w => w.length > 3);
  const inpWords = inp.split(/\s+/).filter(w => w.length > 3);

  for (const w of inpWords) {
    if (dbT.includes(w)) return true;
  }
  ...
  if (
    (dbT.includes('alquran') || dbT.includes('al-quran') || dbT.includes('alquan')) &&
    (
      inp.includes('alquran') ||
      inp.includes('al-quran') ||
      inp.includes('alquan') ||
      inp.includes('quran') ||
      inp.includes('kitabku')
    )
  ) {
    return true;
  }
...
function normalizeProfilText(str) {
  return String(str || '')
    .replace(/&/g, ' dan ')
...
function getCheckedProfilLulusan(container) {
  const root = container || document;

  const values = Array.from(root.querySelectorAll('.rpm-profil:checked'))
    .map(cb => normalizeProfilText(cb.value))
    .filter(Boolean);

  return [...new Set(values)];
}

function matchProfilLulusan(values) {
  if (!values) return [];
  ...
  const candidates = values
    .map(v => normalizeProfilText(v).toLowerCase())
    .filter(Boolean);

  const result = [];

  PROFIL_LULUSAN_OPTIONS.forEach(opt => {
    const optNorm = normalizeProfilText(opt).toLowerCase();

    const found = candidates.some(v => {
      if (!v) return false;
      if (v === optNorm) return true;
      if (optNorm.includes(v) || v.includes(optNorm)) return true;

      const optWords = optNorm.split(' ').filter(w => w.length > 3);
      const valWords = v.split(' ').filter(w => w.length > 3);

      return optWords.some(w => v.includes(w)) || valWords.some(w => optNorm.includes(w));
    });

    if (found && !result.includes(opt)) {
      result.push(opt);
    }
  });
...
const METODE_TEMPLATES = {
  'Project Based Learning (PjBL)': {
    ...
    langkah: [
      {
        judul: 'Pertemuan 1: Penentuan Pertanyaan Mendasar',
        memahami: 'Guru menyampaikan pertanyaan mendasar yang memicu rasa ingin tahu siswa tentang topik proyek. Siswa diajak mengidentifikasi masalah nyata yang relevan dengan kehidupan mereka.',
        mengaplikasikan: 'Siswa bekerja dalam kelompok untuk merancang rencana proyek, menentukan produk yang akan dibuat, dan membagi tugas. Guru memfasilitasi diskusi dan memberikan scaffolding.',
        merefleksikan: 'Siswa merefleksikan pemahaman awal mereka tentang topik proyek dan menuliskan pertanyaan yang ingin mereka jawab melalui proyek ini.'
      },
      {
        judul: 'Pertemuan 2: Mendesain Perencanaan Proyek',
        memahami: 'Siswa mempelajari konsep dan teori yang mendukung proyek mereka melalui berbagai sumber (buku, video, wawancara).',
        mengaplikasikan: 'Siswa membuat timeline proyek, daftar bahan/alat yang dibutuhkan, dan kriteria keberhasilan produk. Guru memonitor perkembangan perencanaan.',
        merefleksikan: 'Siswa mengevaluasi kesiapan mereka untuk memulai proyek dan mengidentifikasi tantangan yang mungkin dihadapi.'
      },
      {
        judul: 'Pertemuan 3-4: Menyusun Jadwal & Monitoring',
        memahami: 'Siswa mulai melaksanakan proyek sesuai rencana. Mereka mengumpulkan data, melakukan eksperimen, atau membuat draft produk.',
        mengaplikasikan: 'Guru melakukan monitoring berkala, memberikan feedback, dan membantu mengatasi hambatan. Siswa mendokumentasikan proses kerja mereka.',
        merefleksikan: 'Siswa mencatat progres yang dicapai, masalah yang dihadapi, dan solusi yang diterapkan.'
      },
      {
        judul: 'Pertemuan 5: Menguji Hasil & Presentasi',
        memahami: 'Siswa menyelesaikan produk proyek dan melakukan uji coba atau validasi hasil.',
        mengaplikasikan: 'Siswa mempresentasikan hasil proyek di depan kelas, menjawab pertanyaan dari teman dan guru. Mereka menjelaskan proses, hasil, dan pembelajaran yang didapat.',
        merefleksikan: 'Siswa melakukan self-assessment dan peer-assessment terhadap kualitas produk dan proses kerja.'
      }
    ]
  },
  ...
  'Problem Based Learning (PBL)': {
    ...
    langkah: [
      {
        judul: 'Pertemuan 1: Orientasi pada Masalah',
        memahami: 'Guru menyajikan masalah autentik dan kompleks yang relevan dengan kehidupan siswa. Masalah disajikan dalam bentuk studi kasus atau skenario nyata.',
        mengaplikasikan: 'Siswa dalam kelompok mengidentifikasi apa yang mereka ketahui (known), apa yang perlu mereka ketahui (need to know), dan ide solusi awal.',
        merefleksikan: 'Siswa menuliskan pertanyaan-pertanyaan kunci yang perlu dijawab untuk memecahkan masalah.'
      },
      {
        judul: 'Pertemuan 2: Organisasi Belajar',
        memahami: 'Siswa melakukan brainstorming hipotesis dan kemungkinan solusi. Mereka mengorganisir ide-ide yang muncul.',
        mengaplikasikan: 'Siswa membagi tugas penelitian, menentukan sumber informasi yang akan digunakan, dan merencanakan strategi pengumpulan data.',
        merefleksikan: 'Siswa mengevaluasi pemahaman mereka tentang masalah dan mengidentifikasi gap pengetahuan.'
      },
      {
        judul: 'Pertemuan 3-4: Penyelidikan Mandiri',
        memahami: 'Siswa secara mandiri atau berkelompok mencari informasi dari berbagai sumber (buku, internet, wawancara, observasi).',
        mengaplikasikan: 'Siswa menganalisis informasi yang didapat, menguji hipotesis, dan mengembangkan solusi. Guru memfasilitasi diskusi dan memberikan scaffolding.',
        merefleksikan: 'Siswa mencatat temuan penting dan merevisi pemahaman mereka berdasarkan informasi baru.'
      },
      {
        judul: 'Pertemuan 5: Analisis & Presentasi Solusi',
        memahami: 'Siswa mensintesis informasi dan mengembangkan solusi final untuk masalah yang diberikan.',
        mengaplikasikan: 'Siswa mempresentasikan solusi mereka dengan argumen yang didukung bukti. Kelompok lain memberikan feedback dan pertanyaan kritis.',
        merefleksikan: 'Siswa merefleksikan proses pemecahan masalah, efektivitas solusi, dan pembelajaran yang didapat.'
      }
    ]
  },
  ...
  'Discovery Learning': {
    ...
    langkah: [
      {
        judul: 'Pertemuan 1: Stimulation (Pemberian Rangsangan)',
        memahami: 'Guru memberikan stimulus berupa pertanyaan, gambar, video, atau fenomena yang memicu rasa ingin tahu siswa. Siswa diajak mengamati dan mencatat hal-hal menarik.',
        mengaplikasikan: 'Siswa dalam kelompok mendiskusikan stimulus dan mengidentifikasi pola atau hubungan yang mereka lihat.',
        merefleksikan: 'Siswa menuliskan pertanyaan-pertanyaan yang muncul dari hasil pengamatan mereka.'
      },
      {
        judul: 'Pertemuan 2: Problem Statement (Pernyataan Masalah)',
        memahami: 'Siswa merumuskan masalah atau hipotesis berdasarkan stimulus yang diberikan.',
        mengaplikasikan: 'Siswa dalam kelompok menyepakati masalah yang akan diselidiki dan merencanakan cara menguji hipotesis mereka.',
        merefleksikan: 'Siswa mengevaluasi apakah masalah yang dirumuskan sudah jelas dan dapat diselidiki.'
      },
      {
        judul: 'Pertemuan 3: Data Collection (Pengumpulan Data)',
        memahami: 'Siswa melakukan eksperimen, observasi, atau penelitian untuk mengumpulkan data yang relevan.',
        mengaplikasikan: 'Siswa mencatat data secara sistematis, menggunakan alat ukur yang tepat, dan memastikan validitas data.',
        merefleksikan: 'Siswa memeriksa kelengkapan dan keakuratan data yang telah dikumpulkan.'
      },
      {
        judul: 'Pertemuan 4: Data Processing (Pengolahan Data)',
        memahami: 'Siswa mengolah dan menganalisis data yang telah dikumpulkan menggunakan metode yang sesuai.',
        mengaplikasikan: 'Siswa membuat tabel, grafik, atau diagram untuk memvisualisasikan data. Mereka mencari pola dan hubungan antar variabel.',
        merefleksikan: 'Siswa mengevaluasi apakah hasil pengolahan data mendukung atau menolak hipotesis awal.'
      },
      {
        judul: 'Pertemuan 5: Verification & Generalization',
        memahami: 'Siswa memverifikasi hasil temuan mereka dengan membandingkan dengan teori atau sumber lain.',
        mengaplikasikan: 'Siswa membuat kesimpulan umum (generalisasi) dari hasil penemuan mereka dan mempresentasikan kepada kelas.',
        merefleksikan: 'Siswa merefleksikan proses penemuan, kesulitan yang dihadapi, dan konsep baru yang mereka pahami.'
      }
    ]
  },
  ...
  'Game-Based Learning': {
    ...
    langkah: [
      {
        judul: 'Pertemuan 1: Pengenalan Konsep & Aturan Permainan',
        memahami: 'Guru memperkenalkan konsep pembelajaran melalui game. Siswa mempelajari aturan, tujuan, dan mekanisme permainan.',
        mengaplikasikan: 'Siswa berlatih memainkan game dalam kelompok kecil untuk memahami alur dan strategi dasar.',
        merefleksikan: 'Siswa menuliskan pemahaman mereka tentang hubungan antara game dan materi pembelajaran.'
      },
      {
        judul: 'Pertemuan 2-3: Bermain & Eksplorasi',
        memahami: 'Siswa bermain game secara intensif, mengeksplorasi berbagai strategi dan pendekatan untuk mencapai tujuan.',
        mengaplikasikan: 'Siswa bekerja sama dalam tim, berdiskusi strategi, dan saling membantu. Guru memonitor perkembangan dan memberikan hints jika diperlukan.',
        merefleksikan: 'Siswa mencatat strategi yang berhasil dan yang gagal, serta pembelajaran yang didapat dari setiap ronde permainan.'
      },
      {
        judul: 'Pertemuan 4: Analisis & Diskusi',
        memahami: 'Siswa menganalisis pengalaman bermain mereka dan mengidentifikasi konsep-konsep pembelajaran yang terkandung dalam game.',
        mengaplikasikan: 'Siswa berdiskusi dalam kelompok tentang hubungan antara mekanisme game dengan materi pelajaran. Mereka membuat catatan konseptual.',
        merefleksikan: 'Siswa merefleksikan bagaimana game membantu mereka memahami materi dengan cara yang menyenangkan.'
      },
      {
        judul: 'Pertemuan 5: Presentasi & Aplikasi',
        memahami: 'Siswa mempresentasikan pembelajaran mereka dari game dan bagaimana konsep tersebut dapat diterapkan dalam konteks nyata.',
        mengaplikasikan: 'Siswa membuat proyek kecil atau menyelesaikan tantangan yang mengaitkan konsep dari game dengan situasi dunia nyata.',
        merefleksikan: 'Siswa melakukan self-assessment tentang pemahaman mereka dan bagaimana game mempengaruhi motivasi belajar mereka.'
      }
    ]
  },
  ...
  'Inquiry Learning': {
    ...
    langkah: [
      {
        judul: 'Pertemuan 1: Merumuskan Pertanyaan Inkuiri',
        memahami: 'Guru menyajikan fenomena atau topik yang memicu rasa ingin tahu. Siswa diajak mengajukan pertanyaan-pertanyaan inkuiri yang mendalam.',
        mengaplikasikan: 'Siswa dalam kelompok memilih satu pertanyaan inkuiri yang akan diselidiki dan merumuskan hipotesis awal.',
        merefleksikan: 'Siswa mengevaluasi apakah pertanyaan mereka cukup fokus dan dapat diselidiki.'
      },
      {
        judul: 'Pertemuan 2: Merancang Penyelidikan',
        memahami: 'Siswa mempelajari metode penelitian yang sesuai untuk menjawab pertanyaan inkuiri mereka.',
        mengaplikasikan: 'Siswa merancang prosedur penyelidikan, menentukan variabel, alat, dan bahan yang dibutuhkan. Guru memberikan feedback terhadap rancangan.',
        merefleksikan: 'Siswa memprediksi hasil yang mungkin didapat dan mengidentifikasi potensi kendala.'
      },
      {
        judul: 'Pertemuan 3-4: Melakukan Penyelidikan',
        memahami: 'Siswa melaksanakan penyelidikan sesuai rancangan, mengumpulkan data dan bukti secara sistematis.',
        mengaplikasikan: 'Siswa bekerja dalam tim, saling membantu, dan mendokumentasikan setiap tahap proses. Guru memfasilitasi dan memberikan scaffolding.',
        merefleksikan: 'Siswa mencatat observasi, temuan tak terduga, dan pertanyaan baru yang muncul selama penyelidikan.'
      },
      {
        judul: 'Pertemuan 5: Analisis & Kesimpulan',
        memahami: 'Siswa menganalisis data dan bukti yang telah dikumpulkan untuk menjawab pertanyaan inkuiri.',
        mengaplikasikan: 'Siswa membuat kesimpulan yang didukung bukti, mempresentasikan hasil penyelidikan, dan menjawab pertanyaan dari audiens.',
        merefleksikan: 'Siswa merefleksikan proses inkuiri, validitas kesimpulan, dan pembelajaran yang didapat tentang cara memperoleh pengetahuan.'
      }
    ]
  },
  ...
  'Cooperative Learning': {
    ...
    langkah: [
      {
        judul: 'Pertemuan 1: Pembentukan Tim & Goal Setting',
        memahami: 'Guru menjelaskan prinsip-prinsip cooperative learning dan membentuk kelompok heterogen (4-5 siswa).',
        mengaplikasikan: 'Siswa dalam tim saling mengenal, menetapkan norma kelompok, dan menyepakati tujuan pembelajaran bersama.',
        merefleksikan: 'Siswa menuliskan harapan mereka terhadap kerja sama tim dan kontribusi yang akan mereka berikan.'
      },
      {
        judul: 'Pertemuan 2: Pembelajaran Konsep Dasar',
        memahami: 'Guru menyampaikan materi dasar atau siswa mempelajari materi melalui sumber yang disediakan.',
        mengaplikasikan: 'Siswa dalam tim menggunakan strategi Jigsaw atau Think-Pair-Share untuk memastikan semua anggota memahami konsep dasar.',
        merefleksikan: 'Siswa mengevaluasi pemahaman individu dan mengidentifikasi bagian yang masih perlu dipelajari.'
      },
      {
        judul: 'Pertemuan 3-4: Tugas Kelompok & Interdependensi',
        memahami: 'Siswa mengerjakan tugas kelompok yang memerlukan kontribusi setiap anggota (interdependensi positif).',
        mengaplikasikan: 'Siswa saling mengajar (peer teaching), saling membantu, dan memastikan semua anggota menguasai materi. Guru memonitor dinamika kelompok.',
        merefleksikan: 'Siswa mencatat bagaimana kerja sama tim membantu pembelajaran mereka dan tantangan yang dihadapi.'
      },
      {
        judul: 'Pertemuan 5: Evaluasi Kelompok & Refleksi',
        memahami: 'Siswa mempresentasikan hasil kerja kelompok atau mengikuti evaluasi individu dan kelompok.',
        mengaplikasikan: 'Siswa melakukan peer evaluation dan group processing, memberikan feedback konstruktif kepada anggota tim.',
        merefleksikan: 'Siswa merefleksikan efektivitas kerja sama tim, keterampilan sosial yang dikembangkan, dan pembelajaran tentang kolaborasi.'
      }
    ]
  }
};
...
function loadCSS() {
  ...
  style.textContent = `
    ...
    .rpm-toast-warning { background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); }
    ...
    @media (max-width: 768px) {
      .rpm-form-grid { grid-template-columns: 1fr; }
      .rpm-actions { flex-direction: column; }
      .rpm-btn { width: 100%; justify-content: center; }
    }
  `;
  ...
  container.innerHTML = `
    <div class="rpm-container">
      <div class="rpm-header">
        <h2>🎯 RPM Spesifik</h2>
        <p>
          Rencana Pembelajaran Mendalam - Template Khusus per Metode Pembelajaran
      ...
      <div id="rpm-form-section">
        ...
        <div class="rpm-section">
          ...
          <small style="display:block; color:#64748b; margin-bottom:15px; font-size:11px;">
            💡 Akan mengambil nama dari fitur Data Peserta Didik sesuai Kelas yang dipilih, lalu dibagi random ke 3 kelompok.
          </small>
          ...
          <div class="rpm-form-group">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
              <label style="font-weight: normal;">
                <input type="checkbox" class="rpm-profil" name="rpm-profil" value="Keimanan dan Ketakwaan">
                Keimanan & Ketakwaan
              </label>
              <label style="font-weight: normal;">
                <input type="checkbox" class="rpm-profil" name="rpm-profil" value="Kewargaan">
                Kewargaan
              </label>
              <label style="font-weight: normal;">
                <input type="checkbox" class="rpm-profil" name="rpm-profil" value="Penalaran Kritis">
                Penalaran Kritis
              </label>
              <label style="font-weight: normal;">
                <input type="checkbox" class="rpm-profil" name="rpm-profil" value="Kreatif">
                Kreatif
              </label>
              <label style="font-weight: normal;">
                <input type="checkbox" class="rpm-profil" name="rpm-profil" value="Kolaborasi">
                Kolaborasi
              </label>
              <label style="font-weight: normal;">
                <input type="checkbox" class="rpm-profil" name="rpm-profil" value="Kemandirian">
                Kemandirian
              </label>
              <label style="font-weight: normal;">
                <input type="checkbox" class="rpm-profil" name="rpm-profil" value="Kesehatan">
                Kesehatan
              </label>
              <label style="font-weight: normal;">
                <input type="checkbox" class="rpm-profil" name="rpm-profil" value="Komunikasi">
                Komunikasi
              </label>
            </div>
          </div>
  ...
  try {
    ...
    try {
      ...
      if (snapKelas.exists()) {
        ...
        console.log(`✅ Ditemukan ${names.length} nama di siswa/${kelasStr}`, names);

        if (names.length > 0) return [...new Set(names)];
      }
    } catch (e) {
      console.warn('Gagal ambil per kelas:', e.message);
    }
    ...
    try {
      ...
      if (snapAll.exists()) {
        ...
        let filtered = allEntries.filter(item => !kelasStr || item.kelas === kelasStr);

        if (filtered.length === 0 && allEntries.length > 0) {
          console.warn(`Kelas ${kelasStr} tidak ada, menampilkan semua ${allEntries.length} siswa sebagai fallback`);
          filtered = allEntries;
        }
      ...
      console.warn('Gagal ambil semua siswa:', e.message);
    }
  } catch (e) {
...
async function handleAutoFillKesiapan(container) {
  ...
  try {
    ...
    if (!namaList || namaList.length === 0) {
      showToast('❌ Tidak ada data peserta didik untuk kelas ' + kelas + '. Coba pilih kelas lain atau cek Data Siswa.', 'error');
      ...
      try {
        ...
        allSnap.forEach(d => {
          const data = d.data();
          const n = data.nama_lengkap || data.nama;

          if (n) fallbackNames.push(n);
        });
...
async function generateCPWithAI(container) {
  ...
  try {
    const prompt = `Buatkan Capaian Pembelajaran (CP) untuk Fase ${fase} (Kelas ${kelasNum}) mata pelajaran ${mapel} sesuai Kurikulum Merdeka. Format output: Tulis dalam format "Elemen: [nama elemen]" diikuti "Deskripsi: [deskripsi capaian]" untuk setiap elemen CP. Pisahkan setiap elemen dengan baris kosong. Contoh: Elemen: Pemahaman Konsep Deskripsi: Siswa mampu memahami dan menjelaskan konsep dasar... Elemen: Keterampilan Proses Deskripsi: Siswa mampu menerapkan keterampilan proses...`;
        ...
        if (matchMapel && matchFase) {
          if (Array.isArray(data.tp_rows) && data.tp_rows.length) {
            ...
            data.tujuan_pembelajaran.forEach((tp) => {
              ...
              items.push({
                value: tp,
                label: data.topik ? `${tp} (${data.topik})` : tp,
                docId: docSnap.id
              });
            });
          }
...
async function generateTPWithAI(container) {
  ...
  try {
    const prompt = `Buatkan 3-5 Tujuan Pembelajaran (TP) yang spesifik dan terukur untuk: Mata Pelajaran: ${mapel} Kelas: ${kelas} Tema: ${tema || '-'} Sub Tema: ${subTema || '-'} Topik (gabungan): ${topik || '-'} Format output: Hanya daftar TP, setiap TP diawali dengan angka (1., 2., dst) dan kalimat "Siswa mampu...". Jangan berikan penjelasan lain.`;
...
function loadTemplatePertemuan(metode) {
  ...
  template.langkah.forEach((langkah) => {
...
async function handleGenerateAI(container) {
  ...
  const userProfilBeforeAI = getCheckedProfilLulusan(container);
  const profilCheckedText = userProfilBeforeAI.length
    ? userProfilBeforeAI.join(', ')
    : '(Belum ada centangan - pilih 3-4 yang paling relevan)';
  ...
  const prompt = `Bertindaklah sebagai Ahli Kurikulum Merdeka dan Pengembang RPM profesional dengan keahlian khusus dalam metode ${metode}. Buatkan Rencana Pembelajaran Mendalam (RPM) SPESIFIK untuk metode ${metode} dengan format JSON valid berdasarkan data:
...
${tpText}
Profil Lulusan yang dicentang user: ${profilCheckedText}
...
- Jika user sudah mencentang Profil Lulusan, WAJIB gunakan pilihan user pada field profil_lulusan. Jika belum, pilih 3-4 Profil Lulusan yang paling relevan.
...
- Pertimbangkan CP dan TP yang sudah ada di atas saat menyusun langkah pembelajaran`;
  ...
  try {
    ...
    const jsonRegex = new RegExp('\u0060\u0060\u0060json\\s*([\\s\\S]*?)\u0060\u0060\u0060', 'i');
    const jsonMatch = aiText.match(jsonRegex) || aiText.match(/\{[\s\S]*\}/);
    ...
    if (parsed.target_peserta_didik) {
      container.querySelector('#rpm-target-peserta-didik').value = parsed.target_peserta_didik;
    }

    if (parsed.sarana_prasarana) {
      container.querySelector('#rpm-sarana-prasarana').value = parsed.sarana_prasarana;
    }
    ...
    if (parsed.profil_lulusan) {
      const aiProfil = matchProfilLulusan(parsed.profil_lulusan);
      const finalProfil = userProfilBeforeAI.length ? userProfilBeforeAI : aiProfil;

      container.querySelectorAll('.rpm-profil').forEach(cb => {
        cb.checked = finalProfil.includes(normalizeProfilText(cb.value));
      });
    }
...
async function handleSimpan(container) {
  ...
  const profilLulusan = getCheckedProfilLulusan(container);

  const langkahPembelajaran = [];

  container.querySelectorAll('.rpm-pertemuan-item').forEach((item) => {
      ...
      return `
        <div class="rpm-item">
          <div class="rpm-item-header">
            <div>
              <div class="rpm-item-title">
                ${d.identitas?.mapel || '-'} - ${d.identitas?.tema || d.identitas?.topik || '-'}${d.identitas?.sub_tema || d.identitas?.subtema ? ' - ' + (d.identitas?.sub_tema || d.identitas?.subtema) : ''}
              </div>
              <div class="rpm-item-meta">
                Kelas ${d.identitas?.kelas || '-'} | Metode: ${d.metode_pembelajaran || '-'} | ${date}
              </div>
            </div>
...
window.editRPM = async function (id) {
  try {
    ...
    document.querySelector('#rpm-kelas').value = d.identitas?.kelas && d.identitas?.fase ? `${d.identitas.kelas}|${d.identitas.fase}` : '';
    ...
    const savedProfilLulusan = matchProfilLulusan(d.tujuan_dan_profil?.profil_lulusan || []);

    document.querySelectorAll('.rpm-profil').forEach(cb => {
      cb.checked = savedProfilLulusan.includes(normalizeProfilText(cb.value));
    });
...
function generateIKTP(tujuanList) {
  if (!tujuanList || tujuanList.length === 0) return [];

  return tujuanList.map((tp, i) => {
    let clean = String(tp)
      .replace(/^\d+\.\s*/, '')
      .replace(/Siswa mampu\s*/i, '')
      .trim();

    return 'IKTP ' + (i + 1) + ': Mampu ' + clean.substring(0, 120);
  });
}

function getLintasDisiplin(mapel) {
  let m = (mapel || '').toLowerCase();

  if (m.includes('pai') || m.includes('agama')) {
    return 'Bahasa Indonesia: Memahami simbol huruf. Seni Rupa: Mewarnai kartu huruf. PPKn: Kerjasama dalam kelompok.';
  }

  if (m.includes('ipas')) {
    return 'Matematika: Pengukuran. Bahasa Indonesia: Laporan hasil.';
  }

  return 'Bahasa Indonesia: Literasi teks. Seni: Visualisasi. PPKn: Kolaborasi.';
}
...
function handleExportWord(container) {
  ...
  let profilLulusan = getCheckedProfilLulusan(container);

  if (!profilLulusan.length) {
    let savedProfil = d.tujuan_dan_profil?.profil_lulusan || [];

    if (!Array.isArray(savedProfil)) {
      savedProfil = String(savedProfil)
        .split(/[\n,;]/)
        .map(s => s.trim())
        .filter(Boolean);
    }

    profilLulusan = matchProfilLulusan(savedProfil);
  }

  d.tujuan_dan_profil.profil_lulusan = profilLulusan;

  let isProfilKosong = profilLulusan.length === 0;
  let isCPKosong = !d.tujuan_dan_profil.cp || String(d.tujuan_dan_profil.cp).trim() === '';
  let isTPKosong = !d.tujuan_dan_profil.tujuan_pembelajaran || d.tujuan_dan_profil.tujuan_pembelajaran.length === 0;

  if (isProfilKosong) {
    showToast('⚠️ Profil Lulusan belum dipilih! Centang 3-4 dimensi.', 'error');
  }

  let iktpList = generateIKTP(d.tujuan_dan_profil.tujuan_pembelajaran);
  let lintas = getLintasDisiplin(d.identitas.mapel);
  let judulGabung = [d.identitas.tema, d.identitas.sub_tema].filter(Boolean).join(' - ') || d.identitas.topik || 'RPM Spesifik';

  let fileSafe = (str) => String(str || '')
...
<head>
  ...
  <style>
    ...
    h1 { text-align: center; font-size: 14pt; font-weight: bold; text-transform: uppercase; margin-bottom: 2pt; }
    .subtitle { text-align: center; font-size: 11pt; margin-top: 0; font-style: italic; }
    h2 { font-size: 12pt; font-weight: bold; background: #D9E2F3; mso-shading: #D9E2F3; padding: 4pt 8pt; border: 1pt solid #000; margin-top: 16pt; margin-bottom: 6pt; }
    ...
    th { background: #E7E6E6; mso-shading: #E7E6E6; }
    ...
    .pertemuan-box { border: 1pt solid #000; padding: 8pt; margin-bottom: 12pt; }
    .pertemuan-judul { font-weight: bold; background: #E2EFDA; padding: 4pt; margin: -8pt -8pt 8pt -8pt; border-bottom: 1pt solid #000; }
    .ttd-table { border: none; width: 100%; }
    .ttd-table td { border: none; text-align: center; }
    .ttd-name { font-weight: bold; border-bottom: 1pt solid #000; display: inline-block; min-width: 200px; }
...
<body>
  <div class="Section1">
    ...
    <p class="subtitle">
      <b>${escapeHtml(judulGabung)}</b><br>
      Metode: ${escapeHtml(d.metode_pembelajaran)} | Model: Pembelajaran Mendalam
    </p>

    ${(isProfilKosong || isCPKosong || isTPKosong) ? `
      <div class="warning-box">
        <b>⚠️ Validasi:</b><br>
        ${isCPKosong ? '• CP kosong<br>' : ''}
        ${isTPKosong ? '• TP kosong<br>' : ''}
        ${isProfilKosong ? '• Profil Lulusan belum dipilih (Wajib 3-4)<br>' : ''}
      </div>
    ` : ''}

    <h2>A. IDENTITAS DOKUMEN & INFORMASI UMUM</h2>
    <table>
      ...
      <tr><td class="label-col">Semester / Tahun Ajaran</td><td>Ganjil / 2025-2026</td></tr>
      <tr><td class="label-col">Tema / Sub Tema</td><td>${escapeHtml(judulGabung)}</td></tr>
      <tr><td class="label-col">Alokasi Waktu</td><td>${escapeHtml(d.identitas.alokasi_waktu || '-')}</td></tr>
      <tr><td class="label-col">Model & Metode</td><td>Pembelajaran Mendalam - ${escapeHtml(d.metode_pembelajaran)}</td></tr>
      <tr><td class="label-col">Sarana & Prasarana</td><td>${nl2brSafe(d.sarana_prasarana)}</td></tr>
      <tr><td class="label-col">Kemitraan</td><td>Orang tua mendampingi di rumah, kolaborasi dengan guru kelas</td></tr>
    </table>

    <h2>B. DESAIN AWAL - ANALISIS KESIAPAN MURID</h2>
    <p><b>Instrumen Diagnostik:</b> Observasi awal, tes lisan, tes tulisan awal sesuai Fase ${escapeHtml(d.identitas.fase)}.</p>
    <p><b>Target Peserta Didik:</b> ${nl2brSafe(d.target_peserta_didik)}</p>

    <table>
      <tr>
        <th style="width:15%;">Kelompok</th>
        <th>Karakteristik & Kebutuhan Belajar</th>
      </tr>
      <tr><td class="label-col">🔴 Belum Siap</td><td>${nl2brSafe(d.analisis_kesiapan.belum_siap)}</td></tr>
      <tr><td class="label-col">🟡 Siap / Berkembang</td><td>${nl2brSafe(d.analisis_kesiapan.siap)}</td></tr>
      <tr><td class="label-col">🟢 Mahir</td><td>${nl2brSafe(d.analisis_kesiapan.mahir)}</td></tr>
    </table>

    <h2>C. DESAIN PEMBELAJARAN</h2>

    <h3>1. Capaian Pembelajaran (CP) - Fase ${escapeHtml(d.identitas.fase)}</h3>
    <p>${isCPKosong ? '<span style="color:red;">[CP BELUM DIISI]</span>' : nl2brSafe(d.tujuan_dan_profil.cp)}</p>

    <h3>2. Tujuan Pembelajaran (TP) - Format ABCD</h3>
    <ul>${listToHtml(d.tujuan_dan_profil.tujuan_pembelajaran)}</ul>

    <h3>3. Indikator Ketercapaian Tujuan Pembelajaran (IKTP)</h3>
    <ul>${listToHtml(iktpList, '<li style="color:red;">IKTP akan otomatis terisi setelah TP diisi</li>')}</ul>

    <h3>4. Dimensi Profil Lulusan (Wajib 3-4 Dimensi)</h3>
    ${
      isProfilKosong
        ? '<p><span style="color:red; background:yellow; font-weight:bold;">[BELUM DIPILIH - CENTANG 3-4 PROFIL DI FORM]</span></p>'
        : `<ul>${profilLulusan.map(p => `<li>${escapeHtml(p)} - Relevansi dengan materi ${escapeHtml(judulGabung)}</li>`).join('')}</ul>`
    }

    <h3>5. Lintas Disiplin Ilmu</h3>
    <p>${escapeHtml(lintas)}</p>

    <h2>D. LANGKAH PEMBELAJARAN MENDALAM</h2>
    <p><i>Prinsip: Berkesadaran (Mindful), Bermakna (Meaningful), Menggembirakan (Joyful)</i></p>

    ${d.langkah_pembelajaran.map((p, i) => `
      <div class="pertemuan-box">
        <div class="pertemuan-judul">${escapeHtml(p.judul || 'Pertemuan ' + (i + 1))}</div>
        <p><b>A. Kegiatan Pendahuluan (Berkesadaran - 10'):</b> Salam, doa, apersepsi, pertanyaan pemantik.</p>
        <p><b>B. Memahami (Mindful):</b><br>${nl2brSafe(p.memahami)}</p>
        <p><b>C. Mengaplikasikan (Meaningful & Joyful):</b><br>${nl2brSafe(p.mengaplikasikan)}</p>
        <p><b>D. Merefleksikan:</b><br>${nl2brSafe(p.merefleksikan)}</p>
        <p><b>E. Penutup:</b> Penguatan, reward, doa, tugas kemitraan.</p>
      </div>
    `).join('')}

    <h2>E. DESAIN ASESMEN HOLISTIK</h2>
    <table>
      <tr>
        <th>Jenis</th>
        <th>Deskripsi</th>
      ...
      <tr><td class="label-col">Rubrik 4 Level</td><td>${nl2brSafe(d.asesmen.rubrik_penilaian)}</td></tr>
    </table>

    <h2>F. DIFERENSIASI PEMBELAJARAN - LENGKAP</h2>
    <table>
      <tr>
        <th>Jenis</th>
        <th>Strategi</th>
      </tr>
      <tr><td class="label-col">Konten</td><td>Belum Siap: disederhanakan & konkret. Siap: standar. Mahir: pengayaan.</td></tr>
      <tr><td class="label-col">Proses</td><td>Belum Siap: individual & tutor sebaya. Mahir: jadi tutor.</td></tr>
      <tr><td class="label-col">Produk</td><td>Pilihan: Poster, Video, LKPD.</td></tr>
      <tr><td class="label-col">Remedial</td><td>${nl2brSafe(d.diferensiasi.remedial)}</td></tr>
      <tr><td class="label-col">Pengayaan</td><td>${nl2brSafe(d.diferensiasi.pengayaan)}</td></tr>
    </table>

    <h2>G. REFLEKSI GURU DAN MURID</h2>
    <table>
      <tr><td class="label-col" style="width:20%;">Refleksi Murid</td><td>${nl2brSafe(d.refleksi.siswa)}</td></tr>
      <tr><td class="label-col">Refleksi Guru</td><td>${nl2brSafe(d.refleksi.guru)}</td></tr>
    </table>

    <h2>H. LAMPIRAN</h2>
    <table>
      <tr><td class="label-col" style="width:20%;">LKPD</td><td>${nl2brSafe(d.lampiran.lkpd)}</td></tr>
      ...
      <table class="ttd-table">
        <tr>
          <td>
            Mengetahui,<br>
            Kepala Sekolah<br>
            ${escapeHtml(d.identitas.sekolah)}<br><br><br><br>
            <div class="ttd-name">${escapeHtml(d.tanda_tangan.kepala_sekolah.nama)}</div><br>
  ...
  link.href = url;
  link.download = `RPM_Spesifik_${fileSafe(d.metode_pembelajaran)}_${fileSafe(d.identitas.mapel)}_${fileSafe(d.identitas.tema || d.identitas.topik || 'tema')}.doc`;
  ...
  showToast('📥 Word Presisi Lengkap (Desain Awal + IKTP + Profil) berhasil diunduh!');
}

function gatherFormData(container) {
  const kelasVal = container.querySelector('#rpm-kelas')?.value || '';

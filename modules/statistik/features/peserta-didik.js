// features/peserta-didik.js - Statistik Peserta Didik
...
export async function init(container, db) {
  container.innerHTML = `
    <div style="background:white; border-radius:12px; padding:20px; border:1px solid #e2e8f0;">
      <div style="display:flex; align-items:center; gap:12px; margin-bottom:20px; border-bottom:1px solid #f1f5f9; padding-bottom:16px;">
        ...
        <div>
          <h2 style="margin:0; color:#1e293b; font-size:20px;">Statistik Peserta Didik</h2>
          <p style="margin:4px 0 0 0; color:#64748b; font-size:13px;">Sebaran siswa per kelas, gender, umur, dan mutasi</p>
        </div>
      </div>
  ...
  try {
    ...
    let data = [];
    try {
      const q = query(collection(db, "peserta-didik"), orderBy("createdAt", "desc"));
      const snap = await getDocs(q);
      data = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    } catch (e) {
      console.log("Koleksi peserta-didik belum ada, pakai dummy:", e.message);
    }
  ...
  function renderStats(list) {
    ...
    if (list.length > 0) {
      ...
      tableDiv.innerHTML = `<div style="padding:16px; background:#fffbeb; border:1px solid #fde68a; border-radius:8px; color:#92400e; font-size:13px;">Belum ada data di koleksi <code>peserta-didik</code>. Data dummy statistik ditampilkan di atas. Silakan input data melalui Dapodik atau modul admin.</div>`;
    }
  }
}
